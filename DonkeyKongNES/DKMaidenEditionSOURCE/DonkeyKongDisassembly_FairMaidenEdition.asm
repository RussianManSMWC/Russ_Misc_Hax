...
;places Link from The Legend of Zelda in place of Jumpman (Mario) to rescue Pauline.
;ideas:
;-no jumpimg (OG remix) (DONE!!!!!11111)
;-alter LVL-design of the 75M so it can be actually played? (extend ledges, make lifts check for if the player has been falling for how long (cuz I cant imagine being able to seemlessly walk from platform onto the sprite)
;-limited sword attacks in place of hammers (optional stuff...) (DONE!!!!!11111)
;could be toggles in the main menu, maybe?
;search for !FAIR_MAIDEN tag for all the changes

incsrc Defines.asm
incsrc Defines_FairMaiden.asm			;hacky defines
...
;!FAIR_MAIDEN
;adjusted some platform hitboxes to prolong them (this is for when I finish editing the level...)
DATA_C20C:
db $0E,$D8,$18
db $0E,$C8,$04
db $86,$C8,$04
...
;!FAIR_MAIDEN: remove hammers (swords) from the level
;hammer 1
...
LDA #Sword_HowManyUses				;!FAIR_MAIDEN
STA Sword_Count					;all the option stuff is to be added

LDA #$00
STA SwordCount_OAM_Prop				;this is also for fair maiden (sword counter prop init, so we don't set it every frame)
STA SwordCount_OAM_Prop+4

LDA #Sword_Prop
STA Sword_OAM_Prop
STA Sword_OAM_Prop+4				;wrong palette on init

LDA #Sword_Count_SwordTile			;this is to be added to some init, since we don't need this every frame (done)
STA SwordCount_OAM_Tile+4			;sword counter sword icon
...
;STA Hammer_CanGrabFlag				;!FAIR_MAIDEN
...
INC PhaseNo					;!FAIR_MAIDEN - temporarily disable 75M
JMP CODE_D0B5					;
...
;GetPlayerInput_D175
CODE_D175:
LDA Timer_Sword					;!FAIR_MAIDEN - can't change direction while the sword is out
BNE RETURN_D199
...
STA Player_ButtonPress				;!FAIR_MAIDEN
...
;!FAIR_MAIDEN
;check if player pressed a customized button to bring out the sword
LDA Sword_Count					;check if can bring a sword to begin with
BEQ @Re

LDA Player_ButtonPress				;check if the player pressed a button to get their sword out
AND #Sword_OutButton
BEQ @Re

DEC Sword_Count					;one more sword used

LDA #SwordOutTime
STA Timer_Sword

;moved this code from CODE_D87D
LDA Sound_Music					;
...
LDA Jumpman_OAM_X				;
STA Sword_OAM_X					;initialize sword's position to be at the player's

LDA PhaseNo					;check phase
CMP #Phase_100M					;100M?
BNE @Re						;if not, return
...
STA $00						;
;JSR CODE_D800					;check if can grab a hammer (there are no hammers/swords in the stage itself)
...
;!FAIR_MAIDEN
;this is useless (one, can't even jump, 2, need to activate the sword at any given moment, not when landing)
...
;!FAIR_MAIDEN
;removed this unused block of code for space
;----------------------------------------------
;!UNUSED
...
;!FAIR_MAIDEN
CODE_D6C6:
LDA Timer_Sword					;check for sword timer
BNE @Run
JMP CODE_D7BF					;this is pretty far away

@Run
;the sword will extend, based on the player's direction, and interact with baddies
;at this point I just deleted the old code, you can check the original disassembly at CODE_D6C6 to see what the original was (I did leave bits of the original code so it works)
LDA Sword_Position
CMP #Sword_MaxExtension
BCS @NoExtension

LDA Direction_Horz
LSR						;check if player is facing left or right (cuz nintendo didn't add a special
TAY						;hopefully this isn't needed later on
LDA Sword_OAM_X
CLC
ADC DATA_C79C,y
STA Sword_OAM_X
...
;from 0 to 9
Sword_Count_NumberTiles:
db $E0,$E1,$E2,$E3,$E4,$E5,$E6,$E7,$E8,$E9

CODE_D73E:
LDA #Link_GFXFrame_Sword			;show link's swording frame
JSR CODE_F070					;draw frame

;draw the counter just above the player
LDX Sword_Count
LDA Sword_Count_NumberTiles,x
STA SwordCount_OAM_Tile

LDA Jumpman_OAM_X
STA SwordCount_OAM_X
CLC
ADC #$08
STA SwordCount_OAM_X+4

LDA Jumpman_OAM_Y
SEC
SBC #$08
STA SwordCount_OAM_Y
STA SwordCount_OAM_Y+4

LDA Sword_OAM_X					;since player is stationary, the sword's position is consistent relatively (it does extend directionally)
STA $00
...
CODE_D7AD:
LDA #<Sword_OAM_Y				;first hammer-err, I mean sword OAM (there can be only one sword on screen)
...
;LDA #$00					;
;STA Hammer_CanGrabFlag+1			;(i'll probably have to do something about it, since the player can just use B (or A) to activate the sword for a limited about of times
...
;change player's GFX
LDA #Link_GFXFrame_Standing			;not swording anymore
JSR CODE_F070					;draw frame
...
LDA #$FF
STA SwordCount_OAM_Y
STA SwordCount_OAM_Y+4				;don't show the counter anymore
RTS						;
...
CODE_D800:
;!FAIR_MAIDEN
;Removed the code, since hammers (now swords) aren't in the levels themselves anymore
...
CODE_ECE8:
;!FAIR_MAIDEN
;altered the collision so it's not either barrels or flame boys (that way we can call a dedicated routine from flame guys)
JSR GetSwordHitbox
...
BNE Sword_DestroyingBarrel			;happened!
...
CODE_ED07:
LDA #$3C					;get hammer's (actually swords) hitbox
JMP CODE_EFE8					;

;check hammer<->flame enemy interaction
CODE_ED34:
;!FAIR_MAIDEN
LDA Hammer_DestroyingEnemy			;already destroying a hammer?
BEQ @CheckSword					;stop right there, criminal scum
RTS
...
CODE_ED57:
LDA #$01					;1 - destroyed a flame enemy
BNE SwordCollision_StoreResult

Sword_DestroyingBarrel:
LDA #$00					;0 - destroyed a barrel

SwordCollision_StoreResult:
STA $02
STA Enemy_Destruct_WhichEnemy
...
JSR JumpmanPosToScratch_EAE1
LDA #$4C					;
JSR CODE_EFE8					;reload hitbox for the player (cuz it was probably overwritten by a barrel) (DOESNT WORK!!!!!!!!!!!!)
...
;!FAIR_MAIDEN
;can actually kill an enemy with a sword (since it can be activated at will)
...
;!FAIR_MAIDEN
;checking for a phase is bad
LDA Enemy_Destruct_WhichEnemy
BNE CODE_EE26
...
LDA Enemy_Destruct_WhichEnemy			;flame enemy shouldn't reset color palette
BNE CODE_EE5C					;only barrel
...
;!FAIR_MAIDEN
;more straightforward sword removal

LDA #$00					;reset sword position for the next time the sword extends
STA Sword_Position
...
;!FAIR_MAIDEN
;change palette to match link's GFX

;before
;db $0F,$02,$36,$16				;sprite palettes
;after
db $0F,$29,$27,$07				;link's palette
...
;!FAIR_MAIDEN
;edit title screen to move DK logo and add "Fair Maiden Edition" title between logo and options
FairMaiden_DKLogoDisp = $40			;how much to move DK title (4 8x8 tiles up, i think?)

db $20,$83-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$84-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$85-FairMaiden_DKLogoDisp
db $01
db $62

db $21-$01,$05-FairMaiden_DKLogoDisp
db $01
db $62

db $20,$A6-FairMaiden_DKLogoDisp
db $03|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$88-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$89-FairMaiden_DKLogoDisp
db $01
db $62

db $21-$01,$09-FairMaiden_DKLogoDisp		;moments like this make me wish I had access to a function like .dbyt like in CC65
db $01
db $62

db $20,$8A-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$8C-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$AD-FairMaiden_DKLogoDisp
db $C2
db $62

db $20,$CE-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$8F-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$91-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$B2-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$B3-FairMaiden_DKLogoDisp
db $01
db $62

db $20,$94-FairMaiden_DKLogoDisp
db $01
db $62

db $20,$F3-FairMaiden_DKLogoDisp
db $01
db $62

db $21-$01,$14-FairMaiden_DKLogoDisp
db $01
db $62

db $20,$96-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$97-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat
db $62

db $20,$D7-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat
db $62

db $21-$01,$17-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat
db $62

db $20,$9A-FairMaiden_DKLogoDisp
db $03|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$DB-FairMaiden_DKLogoDisp
db $03|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $20,$9C-FairMaiden_DKLogoDisp
db $03|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$47-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$68-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$69-FairMaiden_DKLogoDisp
db $01
db $62

db $21,$4A-FairMaiden_DKLogoDisp
db $01
db $62

db $21,$A9-FairMaiden_DKLogoDisp
db $01
db $62

db $21,$CA-FairMaiden_DKLogoDisp
db $01
db $62

db $21,$4C-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$4D-FairMaiden_DKLogoDisp
db $01
db $62

db $21,$CD-FairMaiden_DKLogoDisp
db $01
db $62

db $21,$4E-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$50-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$71-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$92-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$53-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$55-FairMaiden_DKLogoDisp
db $05|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$56-FairMaiden_DKLogoDisp
db $03|VRAMWriteCommand_Repeat
db $62

db $21,$D6-FairMaiden_DKLogoDisp
db $03|VRAMWriteCommand_Repeat
db $62

db $21,$98-FairMaiden_DKLogoDisp
db $02|VRAMWriteCommand_Repeat|VRAMWriteCommand_DrawVert
db $62

db $21,$97-FairMaiden_DKLogoDisp
db $01
db $62
...
db $21,$C6
db $13
db $0F,$0A,$12,$1B,$24,$16,$0A,$12,$0D,$0E,$17,$24,$0E,$0D,$12,$1D,$12,$18,$17
...
;!FAIR_MAIDEN
;M = L for link (not to be confused with the other L)
db $20,$94
db $0A						;top of lives/bonus/loop counters portion
;db $25,$16,$2A,$26,$27,$28,$29,$2A

db $25,$15,$2A,$26,$27,$28,$29,$2A
db $15,$2D
...
;US (and Gamecube) has 2 bytes of freespace here, JP has 3 (this statement doesn't matter for a romhack, naturally)

;!FAIR_MAIDEN
;TEMPORARILY DISABLED (bc its unfinished)
;incsrc OptionsCode.asm
